...
export function InnovationPrograms() {
  ...
  return (
    <section
      ...
      className="relative px-5 py-8 sm:px-8 lg:px-12 bg-white dark:bg-transparent"
    >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
            {programs.map((program, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex flex-col items-center gap-4 text-center px-2 group cursor-pointer"
                >
                  <div className="grid h-16 w-16 place-items-center rounded-2xl bg-brand-50 dark:bg-[#086638]/20 transition-colors group-hover:bg-brand-100 dark:group-hover:bg-[#086638]/40 shadow-sm dark:shadow-none">
                    <Icon
                      size={32}
                      className="text-brand-600 dark:text-[#4ade80] transition-transform group-hover:scale-110"
                      strokeWidth={1.5}
                    />
                  </div>
